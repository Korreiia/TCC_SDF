...
<div class="formulario">
        <form action="{{ route('atualizarSolicitacao',['id'=>$solicitacaos->id]) }}" method="post">
          @method('put')
            ...
            <input type="text" name="telefone2" value="{{ $solicitacaos->telefone2 }}" placeholder="Telefone de Recado">
            <input type="text" name="endereco" value="{{ $solicitacaos->endereco }}" placeholder="Endereço" required>
            <input type="text" name="cpf" value="{{ $solicitacaos->cpf }}" placeholder="CPF" required>
